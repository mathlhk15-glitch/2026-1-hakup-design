...
## v3.3 (2026-07-12) — 교표(로고) 통합

### 추가
- ① 포털 학교 선택 버튼에 교표 표시
- ② 학교 선택 순간 교표 스탬프 애니메이션(1.3초, prefers-reduced-motion 대응, 복원 시 미실행)
- ③ 내비게이션 학교 배지에 미니 교표
- ④ choice 상담 요약(화면·인쇄) 상단에 교표 + 정식 학교명 헤더 — 상담 포트폴리오 공식 문서화
- ⑥ 동적 파비콘: 선택한 학교의 교표가 브라우저 탭·홈 화면 아이콘으로

### 변경
- 학교 구분색을 교표 기준으로 재정의: 경일고 초록(#128a43) / 경일여고 오렌지레드(#e8501e) — 기존 파랑/로즈 폐기
- 로고 자산: common/img/logo-boys.webp, logo-girls.webp (모든 로고 참조는 onerror 안전 처리)
